...
def linear_evaluation(epochs, batch_size, learning_rate):
    ...
    writer = SummaryWriter('runs/from_scratch_linear_classification')

    # 加载在ImageNet上预训练的ResNet18
    base_model = resnet18(pretrained=False).cuda()  # 设置pretrained=True来加载预训练权重
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if epoch == epochs - 1:
            torch.save(base_model, f'../final_models/from_scratch_model_epoch_{epoch}.pth')
            torch.save(base_model.state_dict(), f'../final_models/from_scratch_state_dict_epoch_{epoch}.pth')
